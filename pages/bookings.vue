  ...
  if(newServices){
    if(route.service){
      const service = services.value?.find(s =>
          (route.service === 'tirerepair' && s.name === 'Tire Inspection & Repair') ||
          (route.service === 'seasonalchangeover' && s.name === 'Seasonal Changeover Tires Only')
      );
      console.log(service)
      price.value = service.discounts[0] ? service.price - (service.price * service.discounts[0]?.percentage / 100) : service.price;
    ...
    <div class="dark:bg-gray-900/10 rounded md:w-2/3 lg:w-1/3 px-1 py-4">
      <div class="flex justify-center items-center flex-col gap-3 w-full">
        ...
        <div class="w-full p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md"
          ...
          <h2 class="text-2xl font-bold text-center mb-6 text-gray-800 dark:text-white">Choose a date for: {{selectedService}}</h2>
          <div class="space-y-6">
        ...
        <div v-if="currentStep === 3">
          <div class="my-3 p-4 rounded dark:text-white space-y-4 ">
            ...
            <button @click.prevent="nextStep" class="bg-vivid-red">Continue ${{ TPMAmount * 45 }}.00</button>
          </div>
        </div>
